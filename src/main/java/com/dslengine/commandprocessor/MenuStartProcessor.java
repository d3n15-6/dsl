...
public class MenuStartProcessor implements CommandProcessor {
    private UIBuilderController controller;

    public MenuStartProcessor() {}

    public MenuStartProcessor(UIBuilderController controller) {
    ...
    public Node process(String line, Pane currentContainer, Map<String, Object> properties, Label statusLabel) throws IllegalArgumentException {
        ...
        // Parsear propiedades "texto" (ej. "texto" "file").
        String propsString = line.substring("menu:".length()).trim();
        Pattern pattern = Pattern.compile("texto \"([^\"]*)\"");
        Matcher matcher = pattern.matcher(propsString);
        if (matcher.find()) {
            menu.setText(matcher.group(1));
        } else {
            throw  new IllegalArgumentException("[] El menú debe tener una propiedad 'texto'.");
        }
        ...
        if (parent instanceof MenuBar) {
            ...
            ((Menu) parent).getItems().add(menu); // Agrega el SubMenu.
        } else {
            throw new IllegalArgumentException("[] El menú debe estar dentro de un MenuBar o Menu.");
        }

        containerStack.push(menu); // Ahora este menu es el contenedor activo.
        properties.put("lastNode", menu);

        if (statusLabel != null) {
            statusLabel.setText("[↪] Menú creado: " + menu.getClass().getSimpleName());
        }
